...
class Twist extends Component {
  ...
  update(e) {
    ...
    this.setState(prevState => ({
      style: {
        ...prevState.style,
        transform: `perspective(${this.settings.perspective}px) rotateX(${this.settings.axis === 'x' ? 0 : values.tiltY*-1}deg) rotateY(${this.settings.axis === 'y' ? 0 : values.tiltX*-1}deg) scale3d(${this.settings.scale}, ${this.settings.scale}, ${this.settings.scale})`,
      }
    }))
    this.updateCall = null
  }
